...
BOOL EXPORT ItiPermInit (void)
   {
   if (NULL == (hheap = ItiMemCreateHeap (4096)))
      return FALSE;

   if (!InitJob ())
      return FALSE;

   ItiMbRegisterListWnd (ShowMessageL, ItiWndDefListWndTxtProc, hmod);
   ItiMbRegisterStaticWnd (ShowMessageS, ShowMessageSProc, hmod);
