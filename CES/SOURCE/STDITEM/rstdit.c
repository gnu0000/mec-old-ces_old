...
static  BOOL  bDfltPricesRpt  = FALSE;
...
USHORT SummarySectionForDefaultPrices  (PSZ pszStdItemKey, PSZ pszUnit, USHORT pszUnitLen)
   ...
   /* -- Do the database query. */
   hqry = ItiDbExecQuery (szQuery, hheap, hmodModule, ITIRID_RPT,
                          ID_QUERY_DEF_LISTING, &usNumCols, &usErr);
...
//      ItiPrtEndReport();
      ...
      { /* -- should only get ONE row back, but .... */
      while( ItiDbGetQueryRow (hqry, &ppsz, &usErr) )  
         {
         /* ----------------------------------------------------------- *\
          * If we are on a new page...                                  *
...
USHORT PrintProc (HWND hwnd, MPARAM mp1, MPARAM mp2, USHORT usCaller_ID)
   ...
   /* ------------------------------------------------------- *\
   \* -- [1] Get all of the values from check and combo boxes.*/

   /* -- Check if we are to include default unit prices. */
   if (1 == (USHORT)(ULONG)WinSendDlgItemMsg (hwnd, DID_DFLT_PRICES,
                                              BM_QUERYCHECK, 0L, 0L))
      {
      szOptSwInclDflt[0] = '/';
      szOptSwInclDflt[1] = OPT_SW_INCL_DFLT_PRICES;
      szOptSwInclDflt[2] = ' ';
      ...
      if ((usCaller_ID == DefaultUnitPriceS)
         ...
         {
         szOptSwInclDflt[3] = OPT_VAL_DFLT_PRICES;
         }
      else
         {
         szOptSwInclDflt[3] = 'y';
            /* Any letter except OPT_VAL_DFLT_PRICES will do here. */
         }
   ...
   if (us > 3) 
      ...
      szOptSwBaseDate[0] = ' ';
      szOptSwBaseDate[1] = '/';
      szOptSwBaseDate[2] = OPT_SW_BASEDATE;
      szOptSwBaseDate[3] = ' ';
      szOptSwBaseDate[4] = '\"';
      szOptSwBaseDate[5] = '\0';

      ItiStrCat (szOptSwBaseDate, szBaseDate, sizeof szOptSwBaseDate);
      ItiStrCat (szOptSwBaseDate, "\" ", sizeof szOptSwBaseDate);
      }
   ...
   if (bUseSelectedOnly)
      ...
      {
      /* -- Get the spec year, if any. */
      us =  WinQueryDlgItemText(hwnd, DID_DATEEDIT,
                                sizeof szSpecYear, szSpecYear);

      if ((us < 2) || (! ItiCharIsDigit(szSpecYear[0]) ) )
         {
//         szErrorMsg[0] = DEV_MSG_CHAR;
//         szErrorMsg[1] = '\0';
//         ItiStrCat (szErrorMsg,
//            "PROBLEM: No SpecYear was selected from the dropdown list",
//            sizeof szErrorMsg);
//         ItiStrCat (szErrorMsg,
//            ", nor was the 'Selected Items Only' box checked. ",
//            sizeof szErrorMsg);
//         ItiStrCat (szErrorMsg,
//            " Please select, check, or cancel.",
//            sizeof szErrorMsg);
// 
//         ItiRptUtErrorMsgBox (hwnd, szErrorMsg);
//
...
//         return (ITIRPTUT_PARM_ERROR);
         ...
         szOptSwSpecYr[0] = ' ';
         szOptSwSpecYr[1] = '/';
         szOptSwSpecYr[2] = OPT_SW_SPEC_YR;
         szOptSwSpecYr[3] = ' ';
         szOptSwSpecYr[4] = '\0';
         ItiStrCat (szOptSwSpecYr, szSpecYear, sizeof szOptSwSpecYr);
   ...
   /* ------------------------------------------------------- *\
   \* -- [2] Put the selected reports on their own cmdLn.     */
   usRow = 0;
   sIdx = LIT_FIRST;
   do
      {
      sIdx = (SHORT)(ULONG)WinSendDlgItemMsg(hwnd, DID_RPT_LIST,
                                      LM_QUERYSELECTION,  
                                      MPFROMSHORT(sIdx), 0L);
      if (sIdx != LIT_NONE)
         {
         pszBuffer = &szBuffer[0];
         usTxtLen = (USHORT)(ULONG)WinSendDlgItemMsg(hwnd, DID_RPT_LIST,
                                              LM_QUERYITEMTEXT,  
                                  MPFROM2SHORT((SHORT) sIdx,
                                               (SHORT) sizeof(szBuffer)),
                                  MPFROMP(pszBuffer) );
         if (usTxtLen != 0)
            {
            pszBuffer = &szBuffer[0];
            // skip over description to get to dll name.
            while ((pszBuffer != NULL)
                    &&(*pszBuffer != '\0') && (*pszBuffer != '#'))
               pszBuffer++;
            pszBuffer++; /* skip past the # marker. */

            ItiStrCat(aszCmdLns[usRow], pszBuffer, sizeof aszCmdLns[usRow]);
            ItiStrCat(aszCmdLns[usRow], " ", sizeof aszCmdLns[usRow]);
            usRow++;

            /* -- Erase buffer for next use. */
            pszBuffer = &szBuffer[0];
            while ((*pszBuffer)
                   && (pszBuffer != &szBuffer[(sizeof szBuffer)-1]))
               *pszBuffer++ = '\0';
   ...
   /* ------------------------------------------------------- *\
   \* -- [3] Append the cmd lns with their option set.        */
   us = 0;
   while((us < MAX_CMDLNS) && (aszCmdLns[us][0] != '\0'))
      {
      if (aszCmdLns[us][DEF_RPT_LOC] == DEF_RPT)
         /* ------------------------------------
          * -- Default Unit Prices Listing Report
          * --  The options that this report uses are:
          * --   /K file   Names a [path]KeyFile of selected items.
          */
         {
         if (bUseSelectedOnly)
            {
      ...
      if (aszCmdLns[us][STD_RPT_LOC] == STD_RPT)
         /* ------------------------------------
          * -- Standard Item Listing Report
          * --  The options that this report uses are:
          * --   /S 9999   Specifies the SpecYear to use.
          * --   /K file   Names a [path]KeyFile of selected items.
          * --
          * --      Note that the /S & /K options have an XOR relationship
          * --      and one of them MUST be specified.
          */
         {
//         if (szOptSwInclDflt[0] != '\0')
//            {
//            ItiStrCat(aszCmdLns[us], szOptSwInclDflt, sizeof aszCmdLns[us]);
//            }

         if (szOptSwSpecYr[0] != '\0')
            {
            // ItiStrCat(aszCmdLns[us], szOptSwSpecYr, sizeof aszCmdLns[us]);
            /* Since the StdItem Rpt does not use the ItiRptUtMath function,*
             * we can use the available connection to query for the keys.   */
            ...
            if ((usCaller_ID == DefaultUnitPriceS)
               ...
               ItiRptUtPrepQuery(szQuery, (QUERY_ARRAY_SIZE + 1),
                                 " \"" ITIRPTUT_KEY_QUERY_SW_STR, szBaseQuery,
                                 szSpecYear);
               }
         ...
         else if (bUseSelectedOnly)
            {
            ItiStrCat(aszCmdLns[us], szOptSwSlctFile, sizeof aszCmdLns[us]);
            }
         else 
            {/* -- Error, missing SpecYr xor Selection... */
            szErrorMsg[0] = DEV_MSG_CHAR;
            szErrorMsg[1] = '\0';
            ItiStrCat (szErrorMsg,
                       "Error, no SpecYr or selection given for StdItem "
                       "Catalog report; none will be printed. ",
                       sizeof szErrorMsg);
            ItiRptUtErrorMsgBox (hwnd, szErrorMsg);

            /* -- ...so do not exec this report dll. */
            aszCmdLns[us][0] = '\0'; 
            }
         }/* -- end of if STD_RPT */


      if (aszCmdLns[us][AVG_RPT_LOC] == AVG_RPT)
         /* -----------------------------------------
          * -- Standard Item PEMETH Averages Report
          * --  The options that this report uses are:
          * --   /B "mmm dd yyyy"  (Required) Specifies the BaseDate to use.
          * --   /S 9999           Specifies the SpecYear to use.
          * --   /K FileName       Names a [path]KeyFile of selected items.
          * --
          * --      Note that the /S & /K options have an XOR relationship
          * --      and one of them MUST be specified.
          */
         {
         /* -- This report MUST have a BaseDate specified. */
         if (szOptSwBaseDate[0] != '\0')
            {
            ItiStrCat(aszCmdLns[us], szOptSwBaseDate, sizeof aszCmdLns[us]);

            if (szOptSwSpecYr[0] != '\0')
               {
               /* - Since the Avgs Rpt does not use the ItiRptUtMath function, we *
                * - can use the available connection to do a query for the keys.  */
               ItiMbQueryQueryText(hmodModule, ITIRID_RPT, ID_QUERY_ITEM_KEYS,
                                   szBaseQuery, sizeof szBaseQuery );
               ...
               szErrorMsg[1] = '\0';
               ItiStrCat (szErrorMsg,
                          "Error, no SpecYr or selection given for the "
                          "PEMETH Averages report; none will be printed. ",
                          sizeof szErrorMsg);
               ItiRptUtErrorMsgBox (hwnd, szErrorMsg);
            ...
            {/* -- Error, missing basedate... */
            szErrorMsg[0] = DEV_MSG_CHAR;
            szErrorMsg[1] = '\0';
            ItiStrCat (szErrorMsg,
                       "Error, no BaseDate given for PEMETH Averages.",
                       sizeof szErrorMsg);
            ItiRptUtErrorMsgBox (hwnd, szErrorMsg);
            /* -- ...so do not exec this report dll. */
            aszCmdLns[us][0] = '\0'; 
      ...
      if (aszCmdLns[us][REG_RPT_LOC] == REG_RPT)
         /* -----------------------------------------
          * -- Standard Item PEMETH Regression Report
          * --  The options that this report uses is:
          * --   /S 9999   Specifies the SpecYear to use.
          * --   /K file   Names a [path]KeyFile of selected items.
          * --
          * --      Note that the /S & /K options have an XOR relationship
         ...
         if (szOptSwSpecYr[0] != '\0')
            ...
            ItiStrCat (szErrorMsg,
                       "Error, no SpecYr or selection given for StdItem "
                       "PEMETH Regression report; none will be printed. ",
                       sizeof szErrorMsg);
            ItiRptUtErrorMsgBox (hwnd, szErrorMsg);
      ...
      if (aszCmdLns[us][CST_RPT_LOC] == CST_RPT)
         /* -----------------------------------------
          * -- Standard Item Cost-Based Esimate Report
          * --  The options that this report uses is:
          * --   /S 9999   Specifies the SpecYear to use.
          * --   /K file   Names a [path]KeyFile of selected items.
          * --
         ...
         if (szOptSwSpecYr[0] != '\0')
            ...
            ItiStrCat (szErrorMsg,
                       "Error, no SpecYr or selection given for StdItem "
                       "Cost-Based Estimate report; none will be printed. ",
                       sizeof szErrorMsg);
            ItiRptUtErrorMsgBox (hwnd, szErrorMsg);
...
VOID EXPORT ItiDllQueryMenuName (PSZ     pszBuffer,   
                                 ...
                                 PUSHORT pusWindowID) 
   {                                                  
   ItiStrCpy (pszBuffer, "~Standard Item Listing", usMaxSize);   
...
MRESULT EXPORT ItiDllrStdItRptDlgProc (HWND   hwnd,     
                                       USHORT usMessage,
                                       MPARAM mp1,
                                       MPARAM mp2)
   {
   ...
   switch (usMessage)
      ...
      case WM_INITDLG:
         ...
         ItiMbQueryQueryText(hmodModule, ITIRID_RPT, ID_QRY_DEFLT_RPT,
                             szQueryRptDesc, sizeof szQueryRptDesc);
         break;
      ...
      case WM_INITQUERY:
         ...
         if ((usCallerID == DefaultUnitPriceS)
             || (usCallerID == DefaultUnitPriceCatL)
             || (usCallerID == DefaultUnitPriceCatS) )
            {
            bDfltPricesRpt = TRUE;

            hhp = ItiMemCreateHeap (MIN_HEAP_SIZE);

            ppsz = ItiDbGetRow1 (szQueryRptDesc, 
                                 hhp, hmodModule, ITIRID_RPT,
                                 ID_QRY_DEFLT_RPT, &us);
            if (ppsz != NULL)
               {
               WinSetDlgItemText(hwnd, DID_RPT_LIST, ppsz[0]);
               ItiFreeStrArray (hhp, ppsz, us);
               }
   ...
   }/* END OF FUNCTION ItiDllrStdItRptDlgProc */
...
//      }
   ...
   while ( (usKeyFlag = ItiRptUtGetNextKey(pszKey,sizeof(szKey)) )
      ...
      {
//      if (szDfltPrc[0] == OPT_VAL_DFLT_PRICES)
//         {
//         SummarySectionForDefaultPrices(szKey, szUnit, sizeof szUnit);
//         }
//      else
//         {
         SummarySectionFor(szKey, szUnit, sizeof szUnit);

         if (bDoFullRpt) /* ...then do full report (default case) */
            {            /* unset by the OPT_SW_SUMMARY_ONLY      */
            PredSuccSectionFor (szKey);
            YieldSectionFor    (szKey, szUnit);  /* szUnit used here */
            }
         else
            {
            /* -- Finishout this section. */
