...
static BOOL DoProjectExport (HWND hwnd);
...
//         ItiMenuGrayMenuItem (hwndMenu, IDM_PRINTER_SETUP, TRUE);
      ...
      case IDM_EDIT_MENU:
         ...
         ItiMenuGrayMenuItem (hwndMenu, IDM_COPY,   TRUE);
...
BOOL ProcessCommand (HWND hwnd, USHORT usCommand, MPARAM mp2)
   ...
   switch (usCommand)
      ...
      case IDM_EXPORT:
         ...
         usWindowID = (UM) WinSendMsg (hwndActive, WM_ITIWNDQUERY, 
                                       MPFROMSHORT (ITIWND_ID), 0);

         if (usWindowID == ProjectCategoryS ||
             usWindowID == ProjectsForJobS  ||
             usWindowID == ProjectItemS)
            {
            DoProjectExport (hwndActive);
            }
         else
      ...
      case IDM_OPEN_JOBPROJECT:
         DoOpenJobProjectDialogBox ();
         break;
...
static BOOL DoProjectExport (HWND hwnd)
   {
   HHEAP    hheap;
   PSZ      pszJobKey = NULL, pszPCN = NULL;
   BOOL     bOK = TRUE;
   char     szQuery [128];
   DLF      dlf;
   USHORT   usError;
   ...
   dlf.pszTitle         = "Export PES Projects";
   dlf.pszInstructions  = "Instructions";
   ItiStrCpy (dlf.szFileName, "\\*.out", sizeof dlf.szFileName);
   ItiStrCpy (dlf.szOpenFile, "", sizeof dlf.szOpenFile);
   ItiStrCpy (dlf.szLastWild, "\\*.out", sizeof dlf.szLastWild);
   ItiStrCpy (dlf.szLastFile, "", sizeof dlf.szLastFile);

   usError = DlgFile (hwnd, &dlf);
   if (usError != TDF_NEWSAVE && usError != TDF_OLDSAVE)
      {
      return FALSE;
      }
   ...
   if (pszJobKey == NULL)
      {
      /* couldn't get a Job Key, so let's try for a PCN */
      pszPCN = WinSendMsg (hwnd, WM_ITIWNDQUERY, 
                           MPFROMSHORT (ITIWND_DATA),
                           MPFROM2SHORT (0, cJobKey));
      sprintf (szQuery, "SELECT JobKey "
                        "FROM JobProject "
                        "WHERE ProjectControlNumber = '%s'",
                        pszPCN);
      pszJobKey = ItiDbGetRow1Col1 (szQuery, hheap, 0, 0, 0);
      if (pszJobKey == NULL)
         return FALSE;
      }

   bOK = -1 != spawnl (P_DETACH, "pesexp.exe", "pesexp.exe", 
                       dlf.szFileName, pszJobKey, NULL);
