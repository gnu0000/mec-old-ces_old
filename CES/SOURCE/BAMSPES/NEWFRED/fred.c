...
USHORT MapCleanup (HHEAP hheapKeys,
                   HHEAP hheapTemp,
                   USHORT uErr,
                   PPSZ ppszJIKeys,
                   PPSZ ppszPCKeys)
...
//   if (ppszJIKeys)
//      if (hheapKeys)
//         ItiDbFreeZStrArray (hheapKeys, ppszJIKeys);
//   if (ppszPCKeys)
//      if (hheapKeys)
//         ItiDbFreeZStrArray (hheapKeys ppszPCKeys);

   if (hheapKeys)
      ItiMemDestroyHeap (hheapKeys);
   if (hheapTemp)
...
BOOL DoItUp (HHEAP hheap, PSZ psz)
   {
   return !ItiDbExecSQLStatement (hheap, psz));
   }
...
USHORT MapJobItemProjItem (PSZ pszJobKey)
   ...
   if (!(hheapKeys = ItiMemCreateHeap (32767)) ||
       !(hheapTemp = ItiMemCreateHeap (32767)))
      return MapCleanup (hheapKeys, hheapTemp, 1, NULL, NULL);
                
   /*--- Get Job Item Keys ---*/
   sprintf (sz, szJI, pszJobKey);
   if (ItiDbBuildSelectedKeyArray2 (hheapKeys, 0, sz, &ppszJIKeys, puJIKeys))
      return MapCleanup (hheapKeys, hheapTemp, 2, NULL, NULL);

   /*--- Get Project & Category Keys ---*/
   sprintf (sz, szPC, pszJobKey);
   if (ItiDbBuildSelectedKeyArray2 (hheapKeys, 0, sz, &ppszPCKeys, puPCKeys))
      return MapCleanup (hheapKeys, hheapTemp, 3, ppszJIKeys, NULL);
   ...
   for (uPCIndex = 0; ppszPCKeys[uPCIndex]; uPCIndex++)
      ...
      for (uJIIndex = 0; ppszJIKeys[uJIIndex]; uJIIndex++)
         ...
         if (!(psz = ItiDbGetRow1Col1(sz, hheapTemp, 0, 0, 0)))
            {
            /*--- make PI and JIPI ---*/
            if (uRet = MakePIAndJIPI (hheapTemp,   pszJobKey,
                                      ppszJIKeys,  ppszPCKeys,
                                      uPCIndex,    uJIIndex))
               return MapCleanup (hheapKeys, hheapTemp, uRet, ppszJIKeys, ppszPCKeys);

            /*--- This is not really needed ---*/
            }
         ItiFreePPSZ (hheapTemp, psz);
         }
      }
   return MapCleanup (hheapKeys, hheapTemp, 0, ppszJIKeys, ppszPCKeys);
   }
