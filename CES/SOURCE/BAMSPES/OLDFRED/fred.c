...
USHORT MapJobItemProjItem (PSZ pszJobKey)
   ...
   if (!(hheap = ItiMemCreateHeap (32767)))
   ...
   for (uPCIndex = 0; ppszPCKeys[uPCIndex]; uPCIndex++)
      ...
      for (uJIIndex = 0; ppszJIKeys[uJIIndex]; uJIIndex++)
         ...
         if (!ItiDbGetRow1Col1(sz, hheap, 0, 0, 0))
            {
            /*--- make PI and JIPI ---*/
            if (uRet = MakePIAndJIPI (hheap,       pszJobKey,
                                      ppszJIKeys,  ppszPCKeys,
                                      uPCIndex,    uJIIndex))
               return MapCleanup (hheap, uRet, ppszJIKeys, ppszPCKeys);
