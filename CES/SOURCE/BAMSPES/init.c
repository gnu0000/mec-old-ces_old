...
/* include references to project window procedures */
...
char szAppName [] = "Data Browser (BAMS/PES)";
...
void Initialize (void)
   ...
   pglobals = ItiGlobInitGlobals (habMainThread, hmqMainThread, hwndAppFrame,
                               hwndAppClient, szAppName, "BAMS/PES",
                                  VERSION_STRING);
...
// -------------------------------------------------------------------
   ...
   hinit.idAccelTable             = BAMSPES_ACCELERATORS;
   hinit.idActionBar              = BAMSPES_MAIN_MENU;
   hinit.pszHelpWindowTitle       = "BAMS/PES Help",
...
BOOL RegisterWindows (void)
   ...
   if (ItiMbRegisterStaticWnd (ProjectsForJobS, ProjectsForJobStaticProc, 0))
      return FALSE;

   if (ItiMbRegisterStaticWnd (ProjectCategoryS, ProjectCategoryStaticProc, 0))
      return FALSE;

   if (ItiMbRegisterStaticWnd (ProjectItemS, ProjectItemsSProc, 0))
      return FALSE;
   ...
   if (ItiMbRegisterListWnd (ProjectsForJobL, ItiWndDefListWndTxtProc, 0))
      return FALSE;

   if (ItiMbRegisterListWnd (ProjectCategoryL, ItiWndDefListWndTxtProc, 0))
      return FALSE;

   if (ItiMbRegisterListWnd (ProjectItemL1, ItiWndDefListWndTxtProc, 0))
      return FALSE;

   if (ItiMbRegisterListWnd (ProjectItemL2, ItiWndDefListWndTxtProc, 0))
      return FALSE;

   if (ItiMbRegisterListWnd (ProjectItemDL, ItiWndDefListWndTxtProc, 0))
      return FALSE;
   ...
   if (ItiMbRegisterDialogWnd (OpenJobProjectD, OpenJobProjectD, OpenJobProjectProc, 0))
      return FALSE;

   if (ItiMbRegisterDialogWnd (ProjectsD, ProjectsD, EditProjectsProc, 0))
      return FALSE;

   if (ItiMbRegisterDialogWnd (ProjectCategoryD, ProjectCategoryD, EditProjectCategoryProc, 0))
      return FALSE;

   if (ItiMbRegisterDialogWnd (ProjectItemD, ProjectItemD, ProjectItemDProc, 0))
      return FALSE;
