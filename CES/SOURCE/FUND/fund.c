...
static CHAR szDllVersion[] = "1.1a0 fund.dll";
...
VOID EXPORT ItiDllQueryMenuName (PSZ      pszBuffer,
                                 ...
                                 PUSHORT  pusWindowID)
   {
   ItiStrCpy (pszBuffer, "~Fund Catalog", usMaxSize);
   *pusWindowID = FundCatS;
   }


MRESULT EXPORT FundCatSProc (HWND     hwnd,
                         USHORT   usMessage,
                         MPARAM   mp1,
                         MPARAM   mp2)
   {
   switch (usMessage)
      ...
      case WM_INITMENU:
         switch (SHORT1FROMMP (mp1))
            ...
            case IDM_EDIT_MENU:
               ...
               bSelected = ItiWndQueryActive (hwnd, FundCatL, NULL);
      ...
      case WM_COMMAND:
         switch (SHORT1FROMMP (mp1))
            {
            case IDM_CHANGE:
               ItiWndDoDialog (hwnd, FundCatL, FALSE);
               break;

            case IDM_ADD:
               ItiWndDoDialog (hwnd, FundCatL, TRUE);
               break;

            case IDM_DELETE:
               ItiWndAutoModifyDb (WinWindowFromID (hwnd, FundCatL), ITI_DELETE);
               break;
            }
         break;
      }
...
MRESULT EXPORT FundDProc (HWND    hwnd,
                             USHORT   usMessage,
                             MPARAM   mp1,
                             MPARAM   mp2)
   {
...
BOOL EXPORT ItiDllInitDll (void)
   {
   if (ItiMbRegisterStaticWnd (FundCatS, FundCatSProc, hmodMe))
      return FALSE;


   if (ItiMbRegisterListWnd (FundCatL, ItiWndDefListWndTxtProc, hmodMe))
      return FALSE;

   if (ItiMbRegisterDialogWnd (FundD, FundD, FundDProc, hmodMe))
      return FALSE;
