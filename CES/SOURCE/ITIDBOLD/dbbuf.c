...
/***********************************************************************/
//BOOL IsAppDlgWindow (HWND hwnd)
//   {
//   HWND hwndP;
//   char szClass [100];
//
//   if (WinQueryAnchorBlock (hwnd) != pglobals->habMainThread)
//      return FALSE;
//
//
//   WinQueryClassName (hwnd, sizeof szClass, szClass);
//   if ((ULONG)(atol (szClass+1)) != (ULONG)WC_FRAME)
//      return FALSE;
//
//   for (hwndP = hwnd; hwndP; hwndP = WinQueryWindow (hwndP, QW_OWNER, 0))
//      {
//      if (hwndP == HWND_DESKTOP)
//         return FALSE;
//      if (hwndP == pglobals->hwndAppFrame)
//         return TRUE;
//      }
//   return FALSE;
//   }
...
//   HWND   hwndEnum;
//
//   WinPostMsg (WinQueryActiveWindow (HWND_DESKTOP, 0), umsg, mp1, mp2);
//
//   /*--- broadcast to all 1st level frames of the main app ---*/
//   WinBroadcastMsg (hwnd, umsg, mp1, mp2, BMSG_FRAMEONLY | BMSG_POST);
//
////   /*--- broadcast to all dialogs of the main app ---*/
////   henum = WinBeginEnumWindows (HWND_DESKTOP);
////   while ((hwndEnum = WinGetNextWindow (henum)) != NULL)
////      {
////      if (IsAppDlgWindow (hwndEnum))
////         WinPostMsg (hwndEnum, umsg, mp1, mp2);
////      }
...
BOOL DbFillStaticBuffer (HBUF hbuf, HDB hdb, HMODULE hmod)
   ...
   if (!hbuf->uRefCount)
      ...
      return DbDelBuf (hbuf, NULL); 
      }
   DbDecCmdRefCount (hbuf);
   ...
   if (!hbuf->uNumCols)
      ItiDbGetColData (hdb, hheapDB2, hmod, ITIRT_DATA,
                       ITIRID_WND, hbuf->bDlgRes, hbuf->uId,
                       &(hbuf->uNumCols), &(hbuf->pcol));

   /* --- tell windows to prepare for new data --- */
   DbSendMsg (hbuf, WM_ITIDBCHANGED,
              MPFROM2SHORT (ITI_STATIC, hbuf->uId), MPFROMP (hbuf));
   ...
   DbSendMsg (hbuf, WM_ITIDBBUFFERDONE,
                   MPFROM2SHORT (ITI_STATIC, hbuf->uNumRows),
                   MPFROMP (hbuf));
   return TRUE;
   }
...
//
      ...
      uSeg++;
      ItiSemReleaseMutex (hbuf->hsemReadMutex);
//this works      } while (bFillingBuf /* && hbuf->uRefCount */);
//NEW1      } while (bFillingBuf && hbuf->uRefCount && !hbuf->uCmdRefCount);
...
//   WinBroadcastMsg (hbuf->hwnd, WM_ITIDBDELETE,
//                    MPFROM2SHORT (hbuf->uType, hbuf->uId),
//                    MPFROMP (hbuf), BMSG_DESCENDANTS | BMSG_POST);
