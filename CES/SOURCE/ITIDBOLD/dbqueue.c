...
//   /* --- if querying tell windows to prepare for new data --- */
//   if (uCmd == ITIDB_FILLLIST || uCmd == ITIDB_FILLSTATIC)
//      DbSendMsg (hbuf, WM_ITIDBGRAY,
//                 MPFROM2SHORT (hbuf->uType, hbuf->uId), MPFROMP (hbuf));



   /*--- if delete buffer cmd, add to head & dec refct, else add to tail ---*/
         ...
         if (pCmdQHead != NULL)
            ...
            switch (pqe->uCmd)
               ...
               case ITIDB_DELETEBUFFER:
                  /* create a thread to kill the buffer */
