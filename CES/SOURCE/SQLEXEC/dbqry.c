...
FILE *DbInit (PSZ pszLogFile,
   ...
   pfLog = _fsopen (pszLogFile, "at", SH_DENYWR);
   if (pfLog == NULL)
      {
   ...
   fprintf (pfLog, "SQLExec version %s\n%s\n", VERSION_STRING, dbinit ());
...
//   ItiSemCreateMutex (NULL, 0, &hsemHDBMutex);
   ...
   if ((dbuse (hdb, pszDatabase)) == FAIL)
      ...
      sprintf (szTemp, 
               "Unable to use the %s database.  You may have supplied\n"
               "an incorrect database name, or the database may be corrupted.",
               pszDatabase);
      DisplayHardError (szTemp, pszLogFile);
      fprintf (pfLog, "Could not use database %s\n", pszDatabase);
      dbclose (hdb);
      dbfreelogin (plogin);
      return NULL;
      }
...
BOOL DbSendQueryPart (PSZ pszPart, USHORT *pusState)
   ...
   while (strlen (pszPart) > MAX_QUERY_SIZE)
      ...
      if ((*pusState = dbcmd (hdb, szTemp)) == FAIL)
         {
         fprintf (stderr,"unable to send dbcmd, uState = %u\n", *pusState);
         return FALSE;
         }
   ...
   if ((*pusState = dbcmd(hdb, pszPart)) == FAIL)
      {
      FreeHDB (hdb);
      fprintf (stderr,"unable to process dbcmd, uState = %u\n", *pusState);
      return FALSE;
      }
...
HQRY DbExecQuery (PSZ     pszQuery,
   ...
   if ((*pusState = dbsqlexec(hdb)) == FAIL)
      {
      FreeHDB (hdb);
      fprintf (stderr,"unable to process dbsqlexec, uState = %u\n", *pusState);
   ...
   if ((*pusState = dbresults(hdb)) != SUCCEED)
      {
      FreeHDB (hdb);
      fprintf (stderr,"Query did not succeed, usState=%u, query = \"%s\"\n", 
               *pusState, pszQuery);
      fprintf (pfLog, "Offending Query: %s\n", pszQuery);
      return NULL;
      }
   *pusNumCols = dbnumcols (hdb);
   ...
   if ((hqry = (HQRY) malloc (sizeof (QRY))) == NULL)
      {
      fprintf (stderr,"Could not allocate memory for an hqry!\n");
      fprintf (pfLog, 
...
BOOL DbExecSQLStatement (PSZ pszStatement)
   ...
   if ((usState = dbresults(hdb)) != SUCCEED)
      {
      FreeHDB (hdb);
      fprintf (stderr,"ERROR: Query did not succeed query = \"%s\"\n", 
               pszStatement);
      fprintf (pfLog, "Offending Query: %s\n", pszStatement);
      return FALSE;
      }
