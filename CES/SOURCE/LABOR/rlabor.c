...
#define  PRT_Q_NAME             "Labor Rpt"

#define  QUERY_ARRAY_SIZE       720
...
static CHAR szDllVersion[] = "1.1a0 rLabor.dll";
...
static  CHAR  szQueryRates  [SIZE_OF_QUERY_RATES + 1] = "";
...
USHORT SetColumnTitles (void)
   ...
   ItiPrtDrawSeparatorLn(CURRENT_LN,acttl[RPT_TTL_6].usDtaCol,
          acttl[RPT_TTL_6].usDtaCol+acttl[RPT_TTL_6].usDtaColWth);

   ItiPrtDrawSeparatorLn(CURRENT_LN,acttl[RPT_TTL_7].usTtlCol,
          acttl[RPT_TTL_7].usTtlCol+acttl[RPT_TTL_7].usTtlColWth);

   ItiPrtDrawSeparatorLn(CURRENT_LN,acttl[RPT_TTL_8].usTtlCol,
          acttl[RPT_TTL_8].usTtlCol+acttl[RPT_TTL_8].usTtlColWth);

   ItiPrtDrawSeparatorLn(CURRENT_LN,acttl[RPT_TTL_9].usTtlCol,
          acttl[RPT_TTL_9].usTtlCol+acttl[RPT_TTL_9].usTtlColWth);

   ItiPrtDrawSeparatorLn(CURRENT_LN,acttl[RPT_TTL_10].usTtlCol,
         acttl[RPT_TTL_10].usTtlCol+acttl[RPT_TTL_10].usTtlColWth);
   ItiPrtIncCurrentLn (); //line feed
...
USHORT ConvertOrderingStr (PSZ pszOrderingStr)
   ...
   switch (pszOrderingStr[0])
      ...
      default:
         ItiStrCpy(pszOrderingStr, " order by 1 ", sizeof szOrderingStr);
         break;
      }/* end switch */
...
USHORT SummarySectionFor (PSZ pszKey, PSZ pszID)
   ...
   ppsz = ItiDbGetRow1 (szQuery, hhprLabor, hmodrLabor, ITIRID_RPT,
                        ID_QUERY_LISTING, &usNumCols);

   if (ppsz != NULL)
      {
      usLn = 10 + ItiStrLen(ppsz[DESC]) / acttl[RPT_TTL_2].usDtaColWth;
      ...
      ItiFreeStrArray(hhprLabor, ppsz, usNumCols);
      }
   else
      return 13;
...
USHORT SubSectionFor (PSZ pszKey, PSZ pszID)
   {
   USHORT usLnCnt;
   BOOL   bLabeled;
   ...
   /* -- Build the query. */
   ItiRptUtPrepQuery(szQuery, (QUERY_ARRAY_SIZE + 1),
                     szQueryRates, pszKey, szOrderingStr);


   /* -- Do the database query. */
   hqry = ItiDbExecQuery (szQuery, hhprLabor, hmodrLabor, ITIRID_RPT,
                          ID_QUERY_RATES, &usNumCols, &usErr);
   ...
   if (hqry == NULL)
      ...
      while( ItiDbGetQueryRow (hqry, &ppsz, &usErr) )  
         ...
         ItiPrtPutTextAt (CURRENT_LN, acttl[RPT_TTL_10].usDtaCol,
                       RIGHT, (acttl[RPT_TTL_10].usDtaColWth | REL_LEFT),
                       ppsz[NDBOR]);
         ItiPrtIncCurrentLn (); //line feed
...
USHORT PrintProc (HWND hwnd, MPARAM mp1, MPARAM mp2)
   ...
   if (bChecked)
      ...
      ItiMbQueryQueryText(hmodrLabor, ITIRID_RPT, ID_QUERY_ALL,
                          szQueryAll, sizeof szQueryAll );


      /* -- Do the database query. */
      hqry = ItiDbExecQuery (szQueryAll, hhprLocal, hmodrLabor, ITIRID_RPT,
      ...
      if (hqry == NULL)
         ...
         while( ItiDbGetQueryRow (hqry, &ppsz, &usErr) )
            {
            ItiRptUtWriteToKeyFile (hAltFile, ppsz[LBR_KEY], NULL);
   ...
   us = ItiRptUtExecRptCmdLn (hwnd, szCmdLn, RPT_LABOR_SESSION);
...
VOID EXPORT ItiDllQueryMenuName (PSZ     pszBuffer,   
                                 ...
                                 PUSHORT pusWindowID) 
   {                                                  
   ItiStrCpy (pszBuffer, "Labor Listing", usMaxSize);   
   *pusWindowID = rLabor_RptDlgBox;                       
   }/* END OF FUNCTION */
...
BOOL EXPORT ItiDllQueryAvailability (USHORT usTargetWindowID, 
   ...
   {                                                          
   if ((usTargetWindowID == rLabor_RptDlgBox)                     
       && (usActiveWindowID == LaborCatS))                  
      return TRUE;                                            
   else                                                       
      return FALSE;                                           
   }/* END OF FUNCTION */
...
MRESULT EXPORT ItiDllrLaborRptDlgProc (HWND     hwnd,     
                                       USHORT   usMessage,
                                       MPARAM   mp1,
                                       MPARAM   mp2)
   {
   ...
   switch (usMessage)
      {
      case WM_INITDLG:
         us = ItiRptUtInitDlgBox (hwnd, RPT_LABOR_SESSION, 0, NULL);
         break;
...
USHORT EXPORT ItiRptDLLPrintReport
   ...
   \* ------------------------------------------------------------------- */
   hhprLabor = ItiMemCreateHeap (MIN_HEAP_SIZE);
...
//      bPrtRpt = TRUE;
   ...
   ItiRptUtLoadLabels (hmodrLabor, &usNumOfTitles, &acttl[0]);
...
//                       szQueryAll, sizeof szQueryAll );
   ...
   ItiMbQueryQueryText(hmodrLabor, ITIRID_RPT, ID_QUERY_LISTING,
                       szQueryListing, sizeof szQueryListing );

   ItiMbQueryQueryText(hmodrLabor, ITIRID_RPT, ID_QUERY_RATES,
                       szQueryRates, sizeof szQueryRates );
